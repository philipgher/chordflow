...
export function SheetMusic({ chord, chordName, progression, currentIndex }: SheetMusicProps) {
  return (
    <div className="bg-card rounded-xl border border-border overflow-hidden">
      <div className="px-4 py-3 border-b border-border">
        <h2 className="font-semibold text-foreground">Sheet Music</h2>
        <p className="text-xs text-muted-foreground mt-0.5">Traditional notation view</p>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="relative h-32 bg-secondary/20 rounded-lg overflow-hidden">
          {/* Treble clef area */}
          <div className="absolute left-2 top-1/2 -translate-y-1/2 text-4xl text-muted-foreground select-none">𝄞</div>

          {/* Staff lines */}
          <div className="absolute inset-0 flex flex-col justify-center pl-12 pr-4">
            {Array.from({ length: STAFF_LINES }).map((_, i) => (
              <div key={i} className="h-[1px] bg-muted-foreground/40 my-[6px]" />
            ))}
          </div>
            ...
            <div className="flex items-end gap-1">
              {chord.notes.map((note, i) => {
                ...
                return (
                  ...
                  >
                    {hasSharp && <span className="absolute -left-3 text-xs text-foreground">♯</span>}
